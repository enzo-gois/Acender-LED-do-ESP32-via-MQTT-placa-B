...
1. **Conexão Estabelecida (`MQTT_EVENT_CONNECTED`):**
   * O ESP32 se inscreve no tópico da **outra placa** (ex: `esp32/tp2`) para escutar comandos.
...
3. **Publicação:** Assim que o estado muda, ela publica imediatamente a mensagem ("on" ou "off") no tópico de saída desta placa (ex: `esp32/tp1`), avisando a rede que houve uma ação.
