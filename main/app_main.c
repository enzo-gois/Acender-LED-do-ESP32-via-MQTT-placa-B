...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event) {
    switch (event->event_id) {
        ...
        case MQTT_EVENT_CONNECTED:
            ...
            esp_mqtt_client_subscribe(event->client, "esp32/tp2", 0);
        ...
        ESP_LOGI(TAG, "MQTT DATA");
            ...
            if (strncmp(event->topic, "esp32/tp2", event->topic_len) == 0) {
...
static void button_task(void *arg) {
    ...
    while (1) {
        ...
        if(level_now != level_before) {
            ...
            esp_mqtt_client_publish(mqtt_client, "esp32/tp1", level_now ? "on" : "off", 0,1,0);
